...
class MageWorx_SeoCrossLinks_Model_Observer_Product
{
    ...
    public function updateProductProperties($observer)
    {
        ...
        $htmlModify = Mage::getSingleton('mageworx_seocrosslinks/crosslink')
            ->replace('product', $html, $maxReplaceCount, $product->getSku());

        if ($htmlModify && strpos($htmlModify, $glue) !== false) {
