...
abstract class MageWorx_SeoXTemplates_Model_Converter_Product extends MageWorx_SeoXTemplates_Model_Converter
{
    ...
    protected function __convert($vars, $templateCode)
    {
        ...
        foreach ( $vars as $key => $params ) {
            foreach ( $params['attributes'] as $attribute ) {
                switch ($attribute) {
                    case 'name':
                        $value = $this->_convertName($attribute);
                        break;
                    case 'category':
                    ...
                    case 'special_price':
                        ...
                        break;
                    default:
                        $value = $this->_convertAttribute($attribute);
                        break;
                }
     ...
     * @return string
     */
    protected function _convertAttribute($attribute)
    {
        $tempValue = '';
        $value     = $this->_item->getData($attribute);
        if ($_attr     = $this->_item->getResource()->getAttribute($attribute)) {
            $_attr->setStoreId($this->_item->getStoreId());
            $tempValue = $_attr->setStoreId($this->_item->getStoreId())->getSource()->getOptionText($this->_item->getData($attribute));
        }
        if ($tempValue) {
            $value = $tempValue;
        }
